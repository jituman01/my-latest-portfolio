  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        >
          <h3 className="text-gray-500 dark:text-gray-400 text-base md:text-lg font-medium mb-2">
            Hey, I'm
          </h3>
          ...
          {/* Vertical Text Loop Slider (Pure CSS) */}
          <div className="flex justify-center lg:justify-start items-center gap-2 text-gray-500 text-base md:text-lg mt-4">
            ...
            <div className="h-[30px] overflow-hidden relative w-[200px] text-left">
              {words.map((word, i) => (
                <span
                  key={i}
                  className={`text-blue-600 dark:text-blue-400 font-bold absolute inset-0 transition-all duration-500 ease-in-out transform ${
                    i === index
          ...
          <p className="text-lg md:text-lg text-gray-600 dark:text-gray-400 font-semibold mt-2">
        ...
        >
          <div className="relative w-60 h-60 sm:w-72 sm:h-72 lg:w-[400px] lg:h-[400px] rounded-full border border-black/5 dark:border-white/10 p-2 bg-white/5 backdrop-blur-sm shadow-2xl">
            ...
            {/* Stat Cards with Pure Tailwind Transitions & CSS Elastic effect via delay */}
            <StatCard
              icon="🎓"
              label="Freshman"
              sub="Bachelor"
              className={`-left-16 top-10 sm:-left-14 transition-all duration-700 cubic-bezier(0.175, 0.885, 0.32, 1.275) delay-500 ${
            ...
            <StatCard
              ...
              label="20+"
              sub="Projects"
              className={`-right-10 top-0 sm:-right-7 transition-all duration-700 cubic-bezier(0.175, 0.885, 0.32, 1.275) delay-700 ${
                animate ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
              }`}
            />
            <StatCard
              ...
              label="Exp"
              sub="Frontend"
              className={`left-2/3 -bottom-7 -translate-x-1/2 transition-all duration-700 cubic-bezier(0.175, 0.885, 0.32, 1.275) delay-1000 ${
                animate ? 'opacity-100 scale-100' : 'opacity-0 scale-0'
              }`}
            />
          </div>
...
const SocialIcon = ({ icon, href }) => (
  <a
    href={href}
    className="w-12 h-12 flex items-center justify-center rounded-full bg-white dark:bg-[#1a1a1a] border border-black/10 dark:border-white/10 shadow-lg backdrop-blur-md transition-all duration-300 transform hover:scale-110 hover:translate-x-1"
  >
    {icon}
  </a>
);
...
const StatCard = ({ icon, label, sub, className }) => (
  ...
  >
    ...
    <div className="text-left">
      <p className="text-black dark:text-white font-bold text-xs leading-none">
        {label}
      </p>
